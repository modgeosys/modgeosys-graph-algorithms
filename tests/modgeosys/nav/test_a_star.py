...
def test_a_star_finds_shortest_path_manhattan_graph1(valid_graph1):
    ...
    assert len(result) == 2
    assert result == [Edge(weight=2.0, node_indices=frozenset({0, 1}), g=2.0, h=3.0),
                      Edge(weight=3.0, node_indices=frozenset({1, 4}), g=5.0, h=0.0)]
...
def test_a_star_finds_shortest_path_manhattan_graph2(valid_graph2):
    ...
    assert len(result) == 3
    assert result == [Edge(weight=1.0, node_indices=frozenset({0, 2}), g=1.0, h=4.0),
                      Edge(weight=1.0, node_indices=frozenset({2, 3}), g=2.0, h=2.0),
                      Edge(weight=1.0, node_indices=frozenset({3, 4}), g=3.0, h=0.0)]
