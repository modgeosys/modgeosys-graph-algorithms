from modgeosys.nav.distance import manhattan_distance, euclidean_distance, least_squares_distance
...
def test_least_squares_distance_between_identical_points_is_zero():
    point = Node((1.0, 2.0))
    assert least_squares_distance(point, point) == 0.0


def test_least_squares_distance_between_points_on_same_axis_is_squared_difference():
    point_a = Node((0.0, 0.0))
    point_b = Node((0.0, 3.0))
    assert least_squares_distance(point_a, point_b) == 9.0


def test_least_squares_distance_follows_euclidean_principle_for_squared_distances():
    point_a = Node((0.0, 0.0))
    point_b = Node((3.0, 4.0))
    assert least_squares_distance(point_a, point_b) == 25.0
